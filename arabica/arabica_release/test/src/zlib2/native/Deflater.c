...
Java_zlib2_zip_Deflater_initIDs(JNIEnv *env, jclass cls)
{
...
Java_zlib2_zip_Deflater_init(JNIEnv *env, jclass cls, jint level,
                                 jint strategy, jboolean nowrap)
{
    //printf("zlib_zip_Deflater_init: level: %i, strategy: %i, nowrap: %i\n", level, strategy, nowrap);
    int ret;
	...
	ret = deflateInit2(strm, level, Z_DEFLATED,
	    ...
	    printf("init: Error OUT OF MEMORY\n");
          ...
          case Z_STREAM_ERROR:
            free(strm);
	    printf("init: Error ILLEGAL ARGUMENT\n");
            //JNU_ThrowIllegalArgumentException(env, 0);
            return jlong_zero;
          default:
            msg = strm->msg;
            free(strm);
	    printf("init: Error INTERNAL ERROR, %i, %s\n",ret, msg);
...
JNIEXPORT void JNICALL
Java_zlib2_zip_Deflater_setDictionary(JNIEnv *env, jclass cls, jlong strm,
                                          jarray b, jint off, jint len)
{
    ...
    case Z_STREAM_ERROR:
        //JNU_ThrowIllegalArgumentException(env, 0);
        printf("setDictionary: Error ILLEGAL ARGUMENT\n");
	break;
    default:
        printf("setDictionary: ERROR INTERAL ERROR\n");
...
Java_zlib2_zip_Deflater_deflateBytes(JNIEnv *env, jobject this,
                                         jarray b, jint off, jint len)
{
  long temp = (*env)->GetLongField(env, this, strmID);
    ...
    if (strm == 0) {
	printf("deflateBytes: Error NULL POINTER EXCEPTION\n");
	fflush(stdout);
        ...
        if ((*env)->GetBooleanField(env, this, setParamsID)) {
            ...
            if (in_buf == 0) {
                //JNU_ThrowOutOfMemoryError(env, 0);
		printf("deflateBytes: Error OUT OF MEMORY\n");
		fflush(stdout);
                return 0;
            }
		...
		updateTime(env, &before, &after);
            ...
            case Z_BUF_ERROR:
                ...
                return 0;
            default:
		printf("deflateBytes: Error: Internal Error\n");
		fflush(stdout);
...
JNIEXPORT jint JNICALL
Java_zlib2_zip_Deflater_getAdler(JNIEnv *env, jclass cls, jlong strm)
{
    return ((z_stream *)jlong_to_ptr(strm))->adler;
}

JNIEXPORT jlong JNICALL
Java_zlib2_zip_Deflater_getBytesRead(JNIEnv *env, jclass cls, jlong strm)
{
    return ((z_stream *)jlong_to_ptr(strm))->total_in;
}

JNIEXPORT jlong JNICALL
Java_zlib2_zip_Deflater_getBytesWritten(JNIEnv *env, jclass cls, jlong strm)
{
    return ((z_stream *)jlong_to_ptr(strm))->total_out;
}

JNIEXPORT void JNICALL
Java_zlib2_zip_Deflater_reset(JNIEnv *env, jclass cls, jlong strm)
{
...
JNIEXPORT void JNICALL
Java_zlib2_zip_Deflater_end(JNIEnv *env, jclass cls, jlong strm)
{
