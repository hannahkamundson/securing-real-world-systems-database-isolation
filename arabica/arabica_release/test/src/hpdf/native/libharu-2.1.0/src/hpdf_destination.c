...
HPDF_EXPORT(HPDF_STATUS)
HPDF_Destination_SetXYZ_1  (HPDF_Destination  dst,
                          int         left,
                          int        top,
                          int         zoom)
{
    ...
    ret += HPDF_Array_AddName (dst,
                HPDF_DESTINATION_TYPE_NAMES[(HPDF_INT)HPDF_XYZ]);
    ret += HPDF_Array_AddReal (dst, (float) left);
    ret += HPDF_Array_AddReal (dst, (float) top);
    ret += HPDF_Array_AddReal (dst, (float) zoom);
