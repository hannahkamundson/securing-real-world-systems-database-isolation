    ...
    jmethodID method, void* address) 
{
  ...
  NaClLog(0, "[j2c_proxy.c: allocate_native_method_bind] proxy_id: %d, bind: %p, original: %p, address of original: %p\n", proxy_id, bind, bind->original_native_method_address, &(bind->original_native_method_address));
...
#error "Unsupported"
  ...
  //NaClLog(0, "[j2c_proxy.c: bda_j2c_proxy] nid: %d\n", nid);
  ...
  //NaClLog(0, "[j2c_proxy.c: bda_j2c_proxy] nid: %d, bind: %p, s: %p, env: %p, jobject: %p, orig: %p, address of orig: %p\n", nid, bind, s, env, classOrObject, bind->original_native_method_address, &(bind->original_native_method_address));
  //fflush(stdout);
  ...
  //printf("[j2c_proxy.c: bda_j2c_proxy] minfo: %p\n", minfo);
  //fflush(stdout);
  ...
  NaClLog(0, "[j2c_proxy.c: j2c_proxy] original native target: %p, content: %p\n", original_native_target, * (void **)original_native_target);
...
#error "Unsupported"
  ...
  if (s != NULL) {
    //printf("[j2c_proxy.c: j2c_proxy] minfo->fullname: %s, its addr: %p, minfo: %p\n", minfo->fullname, minfo->fullname, minfo);
    //fflush(stdout);

    if (agent_options.jniassert) {
      if ((minfo->returnInfo.type == 'L') ||(minfo->returnInfo.type == '[')) {
	if ((jobject)return_eax != NULL) {
	  bda_check_ref_dangling(s, (jobject)return_eax, 0, minfo->fullname);
	}
      }
      ...
      bda_check_local_frame_leak(s); //the frame must be sential.
                              ...
                              void* address, void** new_address_ptr)
{
  ...
  *new_address_ptr = bind->intermediate_proxy_code;
  ...
  NaClLog(0, "[j2c_proxy.c: j2c_proxy_add] bind: %p, original: %p\n", bind, address);
...
static void bda_analyze_native_method(jvmtiEnv *jvmti, struct native_method_bind * bind)
{
  ...
  //printf("[j2c_proxy.c: analyze_native_method] minfo->fullname: %s, its addr: %p, minfo: %p\n", minfo->fullname, minfo->fullname, minfo);
  //fflush(stdout);
